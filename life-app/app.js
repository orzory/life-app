...
window.addEventListener('load', () => {
  ...
  if (foot) {
    foot.innerHTML = ic('lock') + ' · ' + APP_VERSION +
      ' · <span id="fontStatus" style="opacity:.7">字体检测中…</span>' +
      ...
      '</span>';
    // 检测甜甜圈体是否真正生效（iOS PWA 字体坑极多，给用户一个可见状态，便于定位）
    (async () => {
      try {
        if (document.fonts && document.fonts.ready) await document.fonts.ready;
        const ok = document.fonts.check("16px 'Donut'");
        const el = document.getElementById('fontStatus');
        if (el) el.textContent = ok ? '字体Donut已加载' : '字体回退系统默认';
      } catch (_) {}
    })();
  }
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('sw.js?v165').then(reg => {
